...
SET DllContainingTests="%~dp0SkillTracker.WebApi.Load.Tests\bin\Debug\SkillTracker.WebApi.Load.Tests.dll"
...
"%OpenCoverExe%" ^
 ...
 -skipautoprops ^
 -register:user ^
 -output:"%~dp0GeneratedReports\LoadTestCoverageReport.xml"
 @pause
exit /b %errorlevel%

:RunReportGeneratorOutput
"%ReportGeneratorExe%" ^
 -reports:"%~dp0\GeneratedReports\LoadTestCoverageReport.xml" ^
 -targetdir:"%~dp0\GeneratedReports\LoadTestReportGenerator Output"
 @pause
exit /b %errorlevel%

:RunLaunchReport
start "report" "%~dp0\GeneratedReports\LoadTestReportGenerator Output\index.htm"
@pause
exit /b %errorlevel%