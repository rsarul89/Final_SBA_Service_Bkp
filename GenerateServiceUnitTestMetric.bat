...
SET DllContainingTests="%~dp0SkillTracker.Services.Tests\bin\Debug\SkillTracker.Services.Tests.dll"
...
"%OpenCoverExe%" ^
 ...
 -skipautoprops ^
 -register:user ^
 -output:"%~dp0GeneratedReports\ServicesCoverageReport.xml"
 @pause
exit /b %errorlevel%

:RunReportGeneratorOutput
"%ReportGeneratorExe%" ^
 -reports:"%~dp0\GeneratedReports\ServicesCoverageReport.xml" ^
 -targetdir:"%~dp0\GeneratedReports\ServicesReportGenerator Output"
 @pause
exit /b %errorlevel%

:RunLaunchReport
start "report" "%~dp0\GeneratedReports\ServicesReportGenerator Output\index.htm"
@pause
exit /b %errorlevel%